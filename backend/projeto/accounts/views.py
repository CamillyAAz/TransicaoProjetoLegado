...
class FuncionarioViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        return [permissions.IsAuthenticated(), self.IsAdminOrReadOnly()]
...
@extend_schema(
    ...
    description="Endpoint público para registro de novos funcionários. Use is_admin=true para criar um administrador, false ou omitir para criar usuário comum.",
    tags=['Autenticação'],
    request={
        'application/json': {
            'type': 'object',
...
class LoginView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if not email or '@' not in email:
            return Response({'detail': 'Email inválido', 'code': 'invalid_email'}, status=status.HTTP_400_BAD_REQUEST)
        if not password:
            return Response({'detail': 'Senha é obrigatória', 'code': 'password_required'}, status=status.HTTP_400_BAD_REQUEST)

        user = Funcionario.objects.filter(email=email).first()
        if user is None:
            return Response({'detail': 'User not found', 'code': 'user_not_found'}, status=status.HTTP_404_NOT_FOUND)

        if not user.check_password(password):
            return Response({'detail': 'Credenciais inválidas', 'code': 'invalid_credentials'}, status=status.HTTP_401_UNAUTHORIZED)
        ...
        return Response({
            ...
            'user': {
                ...
                'ui_permissoes': user.ui_permissoes,
